<template>
  <section class="container mt-8 mx-auto">
    ...
    <template v-if="filteredNotes.length > 0">
      <div class="grid grid-cols-3 gap-4">
        ...
        >
          ...
          <button class="absolute bottom-0 outline-none right-0 p-2">
            Borrar
          </button>
        </div>
      </div>
    ...
    <template v-else>
      <p>No hay notas con esa busqueda</p>
    </template>
  </section>
</template>
  ...
  computed: {
    filteredNotes() {
      return this.query.trim().length > 0
        ? this.notes.filter(
            note =>
              normalizeString(note.title).includes(
                normalizeString(this.query)
              ) ||
              normalizeString(note.content).includes(
                normalizeString(this.query)
              )
          )
        : this.notes;
